...
export default function InteractiveMap({
  ...
  const handleMapClick = useCallback(
    (e: google.maps.MapMouseEvent) => {
      ...
      // Fetch real-world address from Google Maps Geocoding API for high accuracy
      const apiKey = process.env.NEXT_PUBLIC_GOOGLE_MAPS_API_KEY || process.env.NEXT_PUBLIC_GOOGLE_MAPS_KEY;
      fetch(
        `https://maps.googleapis.com/maps/api/geocode/json?latlng=${clickedLat},${clickedLng}&key=${apiKey}&language=ar`
      )
        .then((res) => res.json())
        .then((data) => {
          ...
          if (detailedAddress) {
            ...
            onLocationSelect(clickedLat, clickedLng, `Pinned Location (${clickedLat.toFixed(4)}, ${clickedLng.toFixed(4)})`);
          }
        })
        .catch((err) => {
